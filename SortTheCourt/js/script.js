...
function pulse() {
	$("#tutorial").removeClass().addClass("pulse animated").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function() {
		$(this).removeClass();
	});
};

$("#invite").bind("DOMSubtreeModified", function() {
	active = true;
	doc("tutorial").style.display = "none";
	if (doc("invite").children[0] == undefined) return;
	src = doc("invite").children[0].src;
	solution("invite", src.includes("good"), src.split("/").pop().split(".")[0]);
});

$("#decline").bind("DOMSubtreeModified", function() {
	active = true;
	doc("tutorial").style.display = "none";
	if (doc("decline").children[0] == undefined) return;
	src = doc("decline").children[0].src;
...
function solution(div, result, pic) {
	...
	if (div != "") remove(div, "drag" + num, "fadeOut");
	else doc("drags").removeChild(doc("drag" + num));
...
function setnewpic() {
	...
	img.src = nextpic;
	img.draggable = "true";
	img.addEventListener("dragstart", function() { drag(event)}, false);
	doc("drags").appendChild(img);
	doc("drag" + num).style.display = "none";
	show("drag" + num, "bounceIn");
	...
	function frame() {
		...
		if (doc("popup").style.display == "none") {
			if (width <= 0.2) {
				clearInterval(id);
				if (num == 1) {
					doc("tutorial").style.display = "block";
					pulse();
					pl = setInterval(dopulse, 1050);
					function dopulse() {
						if (!active) pulse();
						else clearInterval(pl);
					}
				}
...
function drop(ev, div) {
	if (doc(div).children[0] == undefined) {
		ev.preventDefault();
		var data = ev.dataTransfer.getData("text");
		ev.target.appendChild(doc(data));
	}
}
...
function wrong(pic, del) {
	switch (pic) {
		...
		case "undealt": wrong(doc("drag" + num).src.split("/").pop().split(".")[0], 2);
...
function lower() {
	...
	function popframe() {
		...
		if (popwidth <= 0.2) {
			...
			popwidth -= 0.8;
			doc("popbar").style.width = popwidth + "px";
		}
	}
}

function createpopup(pic) {
	if (!active) doc("tutorial").style.display = "none";
...
function ok() {
	if (!clickpop) return;
	if (!active) doc("tutorial").style.display = "block";
...
function update() {
	...
	if (!successful()) {
		...
		if (doc("drag" + (num - 1)) != null) doc("drag" + (num - 1)).src = "";
		if (doc("drag" + num) != null) doc("drag" + num).src = "";
		applyGameOver();
	}
}
